...
export class MemStorage implements IStorage {
  ...
  async updateUser(updateUser: UpdateUser): Promise<User> {
    const existing = this.users.get(updateUser.id);
    if (!existing) {
    ...
    this.users.set(updateUser.id, updated);
    return updated;
  }
  ...
  async createPrompt(insertPrompt: InsertPrompt & { userId: string }): Promise<Prompt> {
    const id = randomUUID();
    const now = new Date();
    const prompt: Prompt = {
      ...insertPrompt,
  ...
  async updatePrompt(updatePrompt: UpdatePrompt & { userId?: string }): Promise<Prompt> {
    ...
    if (updatePrompt.userId && existing.userId !== updatePrompt.userId) {
      throw new Error(`Prompt with id ${updatePrompt.id} does not belong to user`);
    }
  ...
  async getSnippets(userId?: string): Promise<Snippet[]> {
    ...
    if (userId) {
      snippets = snippets.filter((s) => s.userId === userId);
    }
    return snippets;
  }

  async getSnippet(id: string, userId?: string): Promise<Snippet | undefined> {
  ...
  async updateSnippet(updateSnippet: UpdateSnippet & { userId?: string }): Promise<Snippet> {
    ...
    if (updateSnippet.userId && existing.userId !== updateSnippet.userId) {
      throw new Error(`Snippet with id ${updateSnippet.id} does not belong to user`);
    }
  ...
  async updateLink(updateLink: UpdateLink & { userId?: string }): Promise<Link> {
    ...
    if (updateLink.userId && existing.userId !== updateLink.userId) {
      throw new Error(`Link with id ${updateLink.id} does not belong to user`);
    }
    const updated: Link = {
      ...existing,
      ...updateLink,
  ...
  async getGuides(userId?: string): Promise<Guide[]> {
    ...
    if (userId) {
      guides = guides.filter((g) => g.userId === userId);
    }
    return guides;
  }
  ...
  async getGuide(id: string, userId?: string): Promise<Guide | undefined> {
    ...
    if (userId && guide.userId !== userId) {
      return undefined;
    }
    return guide;
  }

  async createGuide(insertGuide: InsertGuide & { userId: string }): Promise<Guide> {
    const id = randomUUID();
    const now = new Date();
    const guide: Guide = {
      ...insertGuide,
      id,
      userId: insertGuide.userId,
      createdAt: now,
      updatedAt: now,
  ...
  async updateGuide(updateGuide: UpdateGuide & { userId?: string }): Promise<Guide> {
    ...
    if (updateGuide.userId && existing.userId !== updateGuide.userId) {
      throw new Error(`Guide with id ${updateGuide.id} does not belong to user`);
    }
    const updated: Guide = {
      ...existing,
      ...updateGuide,
